...
namespace dr4 {
	...
	struct SplineBezierCubic {
		...
		typedef TypedResult<GeometryResult, SplineBezierCubic> MonotoneResult;
		MonotoneResult forceXMonotonicity() const {
			SplineBezierCubic  res = *this;
			if (res.p0.x > res.p3.x)
				return MonotoneResult::Error(GeometryResult::NonMonotoneInX);

			handlesMonotone(res.p1, res.p2, res.p0, res.p3);
			return res;
		}
	...
	private:
		...
		// Scale of autogenerated tangents
		//static constexpr float k() { return 0.5f; }
		static constexpr float k() { return 0.333f; }
		...
		static void handlesMonotone(Pairf& fst, Pairf& snd, const Pairf& pnt0, const Pairf& pnt1){
			// First constrain fst, then snd
			// Lots of cases that really should not happen (e.g. fst.x < pnt0.x) but handled anyway for logical coherence's sake

			if (fst.x > pnt1.x) {
				auto lineres = AnalyticLinef::Create(pnt0, fst);
				if (!lineres)
					fst = pnt0; // Can't resolve line
				else {
					fst.x = pnt1.x;
					fst.y = lineres.value().at(fst.x);
				}
			}
			else if (fst.x < pnt0.x) {
				fst = pnt0;
			}

			if (snd.x > pnt1.x /*should never happen*/) {
				snd = pnt1; 
			}
			else if(snd.x < fst.x){
				auto lineres = AnalyticLinef::Create(snd, pnt1);
				if (!lineres) {
					snd = pnt1;
				}
				else {
					snd.x = fst.x;
					snd.y = lineres.value().at(snd.x);
				}
			}
		}
	...
	typedef TypedResult<GeometryResult, PiecewiseSpline2> Piecewise2MonotoneResult;
